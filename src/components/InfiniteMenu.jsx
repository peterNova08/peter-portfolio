...
const InfiniteMenu = forwardRef(function InfiniteMenu(
  { items = [], scale = 1.0, onActiveItemChange, showOverlay = true },
  ref
) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = index => {
      ...
      setActiveItem(item);
      if (onActiveItemChange) onActiveItemChange(item, itemIndex);
    };
  ...
  // Expose imperative navigation to the parent (Next/Prev buttons, dots, etc.)
  useImperativeHandle(ref, () => ({
    goToIndex(index) {
      const sketch = sketchRef.current;
      if (!sketch || !items.length) return;

      const itemCount = items.length;
      const normalizedIndex = ((index % itemCount) + itemCount) % itemCount;

      let targetVertexIndex = null;
      for (let i = 0; i < sketch.instancePositions.length; i++) {
        if (i % itemCount === normalizedIndex) {
          targetVertexIndex = i;
          break;
        }
      }
      if (targetVertexIndex === null) return;

      const targetPos = vec3.normalize(vec3.create(), sketch.instancePositions[targetVertexIndex]);
      const snapDir = sketch.control.snapDirection;
      const newOrientation = quat.rotationTo(quat.create(), targetPos, snapDir);

      quat.copy(sketch.control.orientation, newOrientation);
      quat.identity(sketch.control.pointerRotation);
      quat.identity(sketch.control._combinedQuat);
      sketch.control.snapTargetDirection = vec3.clone(snapDir);
    },
    goToNext() {
      const currentIndex = items.findIndex(i => i === activeItem);
      this.goToIndex((currentIndex === -1 ? 0 : currentIndex) + 1);
    },
    goToPrev() {
      const currentIndex = items.findIndex(i => i === activeItem);
      this.goToIndex((currentIndex === -1 ? 0 : currentIndex) - 1);
